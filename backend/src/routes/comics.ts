...
router.get("/comics", async (req, res) => {
  ...
  try {
    ...
    const response = await apiGetPaginated<ComicsType>(
      `/comics?apiKey=${API_KEY}&skip=${skip}&limit=${defaultLimit}`,
    );
...
router.get("/comics/:characterId", async (req, res) => {
  ...
  try {
    ...
    const response = await apiGetPaginated<CompleteCharacterType>(
...
router.get("/comic/:id", async (req, res) => {
  ...
  try {
    ...
    const response = await apiGet<PreviewComicType>(
